DECLARE
   vEmpresa FIC_CABEOTRE.EMPRESA%TYPE := '10';
	nNumero FIC_CABEOTRE.NUMERO%TYPE := 2016009925;
   --
...
SELECT T.*, C.* 
FROM ANE_TINTAS T,
     (SELECT CF.EMPRESA, TO_CHAR(CF.IM_FORMU) IM_FORMU, CF.IM_COMPO, C.IM_DESCR, CF.IM_CANTI, TIP_ARTI TIP_ARTI_COM, COD_ARTI COD_ARTI_COM
         FROM PRI_COMPFORM CF, PRI_EQCOTINT C
         WHERE CF.EMPRESA = C.EMPRESA
           AND CF.IM_COMPO = C.IM_IDENT) C
WHERE T.EMPRESA = C.EMPRESA(+)
  AND T.MATECODE = C.IM_FORMU(+);

--T.MATECODE

Q_COMPFORM(vMateCode PRD_HIREOTRF.MATECODE%TYPE) IS
         SELECT CF.EMPRESA, CF.IM_FORMU, CF.IM_COMPO, C.IM_DESCR, CF.IM_CANTI, TIP_ARTI TIP_ARTI_COM, COD_ARTI COD_ARTI_COM
         FROM PRI_COMPFORM CF, PRI_EQCOTINT C
         WHERE CF.EMPRESA = C.EMPRESA
           AND CF.IM_COMPO = C.IM_IDENT
