--------------------------------------------------------
--  DDL for View ANE_ESTRUC_LAQUE_COST_OT
--------------------------------------------------------

  CREATE OR REPLACE FORCE VIEW "ANALESTA"."ANE_ESTRUC_LAQUE_COST_OT" ("EMPRESA", "NUMEOTRE", "LFS", "DESCRIPCION_LFS", "IDENTIFICADOR", "PASADA", "TIP_ARTI", "COD_ARTI", "DESCRIPCION", "GRAMOSM2", "PRECIO", "TRAZA_PRECIO", "TIP_ARTI_OT", "COD_ARTI_OT", "DESCRIPCION_ARTICULO_OT", "CANTIDAD_CIERRE_ALBARAN_M2", "CANTIDAD_CERRADA_OT_M2", "COSTE_FX") AS 
  SELECT M.EMPRESA, M.NUMEOTRE, M.MATBASE LFS, LI.DESCRIPCION DESCRIPCION_LFS, M.IDENTIFI IDENTIFICADOR, M.DEFIPASA PASADA, M.TIP_ARTI, M.COD_ARTI, A.DESCRIPCION, M.GRAMOSM2, 
         ...
         FROM PRD_HICLOTRE M, ARTICULO A, LFS LI, ANALESTA.ANE_ORDENES_DE_TRABAJO_T1 O
            WHERE LI.EMPRESA IN (SELECT VALOR FROM VALORES_LISTA WHERE CODIGO = 'GNR_EMPRACGR')
              AND M.MATBASE = LI.COD_LFS
         ...
         SELECT M.EMPRESA, M.NUMEOTRE, M.MATBASE LFS, LI.DESCRIPCION DESCRIPCION_LFS, M.IDENTIFI IDENTIFICADOR, M.DEFIPASA PASADA, M.TIP_ARTI, M.COD_ARTI, A.DESCRIPCION, M.GRAMOSM2, 
                ...
                (((O.CANTIDAD_CIERRE_ALBARAN_M2 * M.GRAMOSM2) / 1000) * P_UTILIDAD.F_VALOCAMPO(ANALESTA.P_ANE_MERMLAMI_PRAG.F_PRECIO(M.EMPRESA, M.TIP_ARTI, M.COD_ARTI,O.FECHA_MODIFICACION_OT),'PRECIO')),
                (((O.CANTIDAD_CERRADA_OT_M2 * M.GRAMOSM2) / 1000) * P_UTILIDAD.F_VALOCAMPO(ANALESTA.P_ANE_MERMLAMI_PRAG.F_PRECIO(M.EMPRESA, M.TIP_ARTI, M.COD_ARTI,O.FECHA_MODIFICACION_OT),'PRECIO'))) COSTE_FX 
         FROM PRD_HICLHOTR M, ARTICULO A, LFS LI, ANALESTA.ANE_ORDENES_DE_TRABAJO_T1 O
            WHERE LI.EMPRESA IN (SELECT VALOR FROM VALORES_LISTA WHERE CODIGO = 'GNR_EMPRACGR')
              AND M.MATBASE = LI.COD_LFS
              AND M.EMPRESA = A.EMPRESA
              AND M.TIP_ARTI = A.TIP_ARTI
